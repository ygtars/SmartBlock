...
public:
    ...
    bool WriteZXCKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZXCKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZXCKSeed_deprecated(uint256& seed);
    bool EraseZXCKSeed();
    bool EraseZXCKSeed_deprecated();

    bool WriteZXCKCount(const uint32_t& nCount);
    bool ReadZXCKCount(uint32_t& nCount);

...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered smartblock address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 XCK.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal smartblock:URI (smartblock:XyZ...?message=example)
            {
...
static void NotifyzXCKReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXCKReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXCKReset.connect(boost::bind(NotifyzXCKReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXCKReset.disconnect(boost::bind(NotifyzXCKReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zxckTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}

...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zxckTracker->HasMintTx(hash))) {
            //zXCK stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZXCK;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by SMARTBLOCK Address
...
bool IsZXCKType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZXCK:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZXCK || type == TransactionRecord::MNReward) {

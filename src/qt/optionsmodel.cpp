...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::XCK);
    ...
    if (!settings.contains("nAnonymizeSmartBlockAmount"))
        settings.setValue("nAnonymizeSmartBlockAmount", 1000);

    nAnonymizeSmartBlockAmount = settings.value("nAnonymizeSmartBlockAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSmartBlockAmount"))
        SoftSetArg("-anonymizesmartblockamount", settings.value("nAnonymizeSmartBlockAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in smartblock.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSmartBlockAmount:
            return QVariant(nAnonymizeSmartBlockAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSmartBlockAmount:
            nAnonymizeSmartBlockAmount = value.toInt();
            settings.setValue("nAnonymizeSmartBlockAmount", nAnonymizeSmartBlockAmount);
            emit anonymizeSmartBlockAmountChanged(nAnonymizeSmartBlockAmount);
            break;
        case CoinControlFeatures:

...
void PrepareShutdown()
{
    ...
    RenameThread("smartblock-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "smartblock.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "smartblockd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XCK and zXCK money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XCK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XCK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, smartblock, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XCK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SMARTBLOCK specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-xckstake=<n>", strprintf(_("Enable or disable staking functionality for XCK inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zxckstake=<n>", strprintf(_("Enable or disable staking functionality for zXCK inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzxck=<n>", strprintf(_("Enable automatic wallet backups triggered after each zXCK minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zxckbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zXCK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizesmartblockamount=<n>     " + strprintf(_("Keep N XCK anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The SMARTBLOCK Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("smartblock-loadblk");
...
/** Sanity checks
 *  Ensure that SMARTBLOCK is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SMARTBLOCK Core is shutting down."));
...
#endif
    // Make sure only a single SMARTBLOCK process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SMARTBLOCK Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SMARTBLOCK version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //SMARTBLOCK: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //SMARTBLOCK specific: zerocoin and spork DB's
                ...
                // SMARTBLOCK: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZXCKMinted();
                        RecalculateZXCKSpent();
                    }
                    RecalculateXCKSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // SMARTBLOCK: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SMARTBLOCK Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart SMARTBLOCK Core to complete") << "\n";
        ...
        zwalletMain = new CzXCKWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zXCKWallet
        uiInterface.InitMessage(_("Syncing zXCK wallet..."));

        bool fEnableZPivBackups = GetBoolArg("-backupzxck", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zxckTracker->Init();
...
//    }
//
//    nAnonymizeSmartBlockAmount = GetArg("-anonymizesmartblockamount", 0);
//    if (nAnonymizeSmartBlockAmount > 999999) nAnonymizeSmartBlockAmount = 999999;
//    if (nAnonymizeSmartBlockAmount < 2) nAnonymizeSmartBlockAmount = 2;
    ...
    LogPrintf("Anonymize SMARTBLOCK Amount %d\n", nAnonymizeSmartBlockAmount);
    ...
    /* Denominations
       ...
       For example:
       1XCK+1000 == (.1XCK+100)*10
       10XCK+10000 == (1XCK+1000)*10
    */

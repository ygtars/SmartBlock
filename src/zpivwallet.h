...
#ifndef SMARTBLOCK_ZXCKWALLET_H
#define SMARTBLOCK_ZXCKWALLET_H
...
public:
    CzXCKWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZXCK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZXCK(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SMARTBLOCK_ZXCKWALLET_H

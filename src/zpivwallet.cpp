...
CzXCKWallet::CzXCKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxck seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXCKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXCKSeed_deprecated()) {
                    LogPrintf("%s: Updated zXCK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxck seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxck wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXCKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXCKCount(nCountLastUsed);
    else if (!walletdb.ReadZXCKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXCKWallet::Lock()
{
    seedMaster = 0;
}

void CzXCKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXCKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZXCK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzXCKWallet::LoadMintPoolFromDB()
{
...
void CzXCKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXCKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXCKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxckTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXCKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXCK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxckTracker which also adds to database
    pwalletMain->zxckTracker->Add(dMint, true);
...
void CzXCKWallet::SeedToZXCK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzXCKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXCKWallet::GenerateDeterministicZXCK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXCKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXCKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{

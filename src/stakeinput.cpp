...
// The zXCK block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zXCK is a hash of the serial
...
{
    //Create an output returning the zXCK that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZXCKOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zXCK output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zXCK", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZXCKOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zXCK output", __func__);
...
{
    CzXCKTracker* zxckTracker = pwallet->zxckTracker.get();
    CMintMeta meta;
    if (!zxckTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zxckTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a XCK stake is the outpoint

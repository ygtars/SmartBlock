SMARTBLOCK Core version *3.1.0* is now available from:  <https://github.com/smartblock-project/smartblock/releases>

This is a new major version release, including various bug fixes and performance improvements, as well as updated translations.

Please report bugs using the issue tracker at github: <https://github.com/smartblock-project/smartblock/issues>


Mandatory Update
==============

SMARTBLOCK Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zXCK Staking is here! zXCK staking will be activated on the morning of the 8th of May 2018. With the release of zXCK staking, there are effectively 2 versions of zXCK, zXCK minted on the 3.0.6 SMARTBLOCK wallet or lower, and zXCK minted on SMARTBLOCK wallet version or higher. New features in this release will require the use of zXCK v2, zXCK minted on this wallet release 3.1.0 or later. If you currently hold zXCK v1 and wish to take advantage of zXCK staking and deterministic zXCK, you will need to spend the zXCK v1 to yourself and remint zXCK v2.
Note: To find your zXCK version, click the privacy tab, then the zXCK Control button then expand the arrows next to the desired denomination.


### Deterministic zXCK Seed Keys

zXCK is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zXCK outside of the wallet that the zXCK had been minted on. zXCK can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zXCK minting

zXCK minting now only requires 1 further mint (down from 2) to mature. zXCK mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zXCK Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zXCK denomination has been spent or not.



XCK/zXCK Staking and Masternode Rewards
--------------

### XCK, zXCK and Masternode Payment Schedule

To encourage the use of zXCK and increase the SMARTBLOCK zerocoin anonymity set, the SMARTBLOCK payment schedule has been changed to the following:

If a user staking zXCK wins the reward for their block, the following zXCK reward will be: 
- 3 zXCK (3 x 1 denominations) rewarded to the staker, 2 XCK rewarded to the masternode owner and 1 XCK available for the budget. This is a total block reward of 6 XCK, up from 5.

If a user staking XCK wins the reward, the following amounts will be rewarded: 
- 2 XCK to the XCK staker, 3 XCK to the Masternode owner and 1 XCK available for the budget. This is a total block reward of 6 XCK, up from 5.


### Return change to sender when minting zXCK

Previously, zXCK minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most XCK and return the change from a zXCK mint to that address.
...
The visual layout of the SMARTBLOCK Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zXCK minting feature. This is enabled by default and the enablezeromint=0 setting in the smartblock.conf file will overwrite the GUI option.
-	The percentage of autominted zXCK can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular XCK.
...
### Summary
 ...
 The SMARTBLOCK wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the smartblock.conf file, found in the SMARTBLOCK data directory.
- backuppath = <directory / full path>
- zxckbackuppath = <directory / full path>
...
* If zXCK backup, auto generated name is `wallet-autozxckbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
SMARTBLOCK Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The SMARTBLOCK client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running smartblockd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the SMARTBLOCK support group, which often times sees users say "I didn't spend that zXCK". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the SMARTBLOCK masternodes, the monthly budget available to be utilised has been increased to 42,000 XCK / month. This XCK only has the opportunity to be raised once per month (paid to winning proposals) with any unused XCK not created by the blockchain.

### Budget Finalization Fee

The SMARTBLOCK finalization fee for successful proposals has now been reduced, this fee is now 5 XCK down from 50 XCK. The total fee outlay for a successful proposal is now a total of 55 XCK.
...
-	Enforced v1 zXCK spends to require a security level of 100
-	Updates to zXCK spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the SMARTBLOCK-Qt masternodes tab
-	Updated SMARTBLOCK-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and SMARTBLOCK's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zXCK Control dialog in the QT wallet, a user is able to see zXCK marked as version 1 or 2.

zPoS (zXCK staking)
--------------

Once a zXCK has over 200 confirmations it becomes available to stake. Staking zXCK will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zXCK. So for example if a 1,000 zXCK denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zXCKs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within SMARTBLOCK's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zXCK, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zXCK that can be recalculated at any time using the seed. Deterministic zXCK allows for users to backup all of their future zXCK by simply recording their seed and keeping it in a safe place (similar to backing up a private key for XCK). The zXCK seed needs to remain in the wallet in order to spend the zXCK after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zXCK data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

SMARTBLOCK's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which SMARTBLOCK used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to SMARTBLOCK using the bug bounty program. The modulus is now correctly imported and SMARTBLOCK's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zXCK v2: zPoS dzXCK ezXCK external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zXCK balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zXCK status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault smartblock-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zXCK spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zXCK. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zXCK. (presstab)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/smartblock-project-translations/), the QA team during Testing and the Node hosts supporting our Testnet.

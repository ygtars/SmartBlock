SMARTBLOCK Core version 3.0.4 is now available from:
...
SMARTBLOCK Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zXCK protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
- #294 `27c0943` Add additional checks for txid for zxck spend. (presstab)
...
- #327 `aa1235a` [Main] Don't limit zXCK spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zXCK (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when smartblockd --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] SMARTBLOCK not responding on user quitting app (Aaron Langford)
